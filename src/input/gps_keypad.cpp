...
{
    static const bool enabled = envEnabled("CAP_LORA_KEYBOARD_DEBUG", false);
    return enabled;
}
...
bool GpsKeypad::openDefault()
{
#if !LV_USE_SDL && defined(__linux__)
    if (const char* device_path = std::getenv("CAP_LORA_KEYBOARD_DEVICE")) {
    ...
    if (!opened) {
        spdlog::warn("GpsKeypad: no keyboard input device opened");
    }
    return opened;
#else
    return false;
...
bool GpsKeypad::openDevice(const std::string& path, bool require_app_keys)
{
#if !LV_USE_SDL && defined(__linux__)
    if (!ensureIndev()) {
        return false;
    ...
    spdlog::info("GpsKeypad: opened {}{}", path, grab_input ? " with grab" : "");
    return true;
#else
    (void)path;
...
void GpsKeypad::poll()
{
    ...
    for (int fd : _event_fds) {
        while (true) {
            ...
            if (bytes_read < 0) {
                spdlog::warn("GpsKeypad: failed to read input event: {}", std::strerror(errno));
            }
            break;
        }
    }
...
lv_indev_t* GpsKeypad::indev() const
{
    return _indev;
}

void GpsKeypad::setKeyCallback(KeyCallback callback)
{
    _key_callback = std::move(callback);
}

void GpsKeypad::readCb(lv_indev_t* indev, lv_indev_data_t* data)
{
    auto* keypad = static_cast<GpsKeypad*>(lv_indev_get_user_data(indev));
    if (!keypad || !data) {
        return;
    }
...
bool GpsKeypad::ensureIndev()
{
    ...
    if (!_indev) {
        spdlog::error("GpsKeypad: failed to create LVGL keypad indev");
        return false;
    }
...
void GpsKeypad::pushKeyEvent(uint16_t code, int32_t value)
{
    if (value != 0 && value != 1) {
#if !LV_USE_SDL && defined(__linux__)
        if (inputDebugEnabled()) {
            spdlog::info("GpsKeypad: ignored repeat key code={}, value={}", code, value);
        }
#endif
        return;
    }
...
#if !LV_USE_SDL && defined(__linux__)
        if (inputDebugEnabled()) {
            spdlog::info("GpsKeypad: ignored key code={}, value={}", code, value);
        }
#endif
        return;
    }
...
#if !LV_USE_SDL && defined(__linux__)
    if (inputDebugEnabled()) {
        spdlog::info("GpsKeypad: key code={}, translated={}, utf8={}, pressed={}", code, key, debugKeyUtf8(key),
                     pressed);
    }
#endif
    bool consumed = false;
...
uint32_t GpsKeypad::translateKey(uint16_t code) const
{
#if !LV_USE_SDL && defined(__linux__)
...
bool GpsKeypad::shiftPressed() const
{
    return _left_shift_pressed || _right_shift_pressed;
}

const char* GpsKeypad::keyUtf8(uint32_t key) const
{
