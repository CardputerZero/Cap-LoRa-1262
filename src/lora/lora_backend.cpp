...
#include "RadioLib.h"
// The application supplies its own Linux GPIO/SPI implementation below.  The
// RadioLib lgpio HAL is available as an explicit opt-in for deployments that
// link liblgpio; keeping it opt-in avoids an accidental link-time dependency.
...
static bool sx1262_wait_while_busy(unsigned int timeout_ms)
{
    ...
    while (waited_ms < timeout_ms) {
        ...
        int busy = cp0_lora_backend::gpio_get_value_any(g_lora.busy_gpio, g_lora.busy_fd);
...
{
    if (lora_stop_requested()) return false;
    if (cp0_lora_backend::gpio_set_value_any(g_lora.rst_gpio, g_lora.rst_fd, 0) < 0) return false;
    usleep(20000);
    if (cp0_lora_backend::gpio_set_value_any(g_lora.rst_gpio, g_lora.rst_fd, 1) < 0) return false;
    usleep(10000);
...
static bool probe_lora_spi_device(void)
{
    ...
    auto try_probe = [](const char *dev) -> bool {
        ...
        g_lora.spi.close();
        if (cp0_lora_backend::gpio_init_output_any("LORA_RST_CHIP", "LORA_RST_OFFSET", g_lora.rst_gpio, 1, &g_lora.rst_fd, "RST") < 0) {
    ...
    void pinMode(uint32_t pin, uint32_t mode) override
    {
        ...
        if (mode == GpioModeOutput) {
            ...
            if (pin == (uint32_t)g_lora.busy_gpio) {
                (void)cp0_lora_backend::gpio_init_input_any("LORA_BUSY_CHIP", "LORA_BUSY_OFFSET", (int)pin, &g_lora.busy_fd, "BUSY");
            }
        }
    }
...
static void lora_set_diag_step(const char *step, int code, const char *detail)
{
    ...
    // Keep diagnostics bounded even when a driver returns a long error string.
    snprintf(g_lora.last_diag, sizeof(g_lora.last_diag), "%.96s%s%.140s | rc=%d", step ? step : "diag",
...
static void lora_init_hardware(void)
{
    ...
    if (cp0_lora_backend::gpio_init_input_any("LORA_BUSY_CHIP", "LORA_BUSY_OFFSET", g_lora.busy_gpio, &g_lora.busy_fd, "BUSY") < 0) {
        g_lora.initialized = false;
        g_lora.hw_ready    = false;
    ...
    if (cp0_lora_backend::gpio_init_input_irq_any("LORA_IRQ_CHIP", "LORA_IRQ_OFFSET", g_lora.irq_gpio, &g_lora.irq_fd, "IRQ") < 0) {
...
static void copy_cstr(char* destination, size_t capacity, const char* source)
{
...
void get_info(cap_lora::LoraInfo *info, bool drain_events)
{
    ...
    *info = cap_lora::LoraInfo{};
    ...
    copy_cstr(info->spi_device, sizeof(info->spi_device), g_lora.spi.path());
    copy_cstr(info->last_rx, sizeof(info->last_rx), g_lora.last_rx);
    copy_cstr(info->last_tx, sizeof(info->last_tx), g_lora.last_tx);
    copy_cstr(info->diag, sizeof(info->diag), g_lora.last_diag);
    copy_cstr(info->probe_summary, sizeof(info->probe_summary), g_lora.probe_summary);
    copy_cstr(info->probe_display, sizeof(info->probe_display), g_lora.probe_display);
    copy_cstr(info->pi4io_status, sizeof(info->pi4io_status), cp0_lora_pi4io_controller::status());
